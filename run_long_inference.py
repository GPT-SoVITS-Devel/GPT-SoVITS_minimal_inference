        ...
        with torch.no_grad():
            ...
            for i, seg_text in enumerate(segments):
                ...
                def decode_and_crop(tokens, hist, lookahead):
                    ...
                    audio = self.vq_model.decode(full_chunk.unsqueeze(0), curr_phones_tensor, [refer_spec], noise_scale=0, speed=speed, sv_emb=[sv_emb])
...
def launch_webui(args):
    ...
    def predict(ref_audio, ref_text, ref_lang, text, lang, top_k, top_p, temperature, speed, chunk_length):
        ...
        for audio_chunk in gen:
            # Gradio streaming expects (sample_rate, numpy_array)
            # or yielding chunks. For real-time streaming in Gradio 4.x+,
            # we yield (sr, chunk)
...
if __name__ == "__main__":
    ...
    if args.webui:
        ...
        inference = GPTSoVITSLongInference(args.gpt_path, args.sovits_path, args.cnhubert_base_path, args.bert_path)
        full_audio = []
        for audio_chunk in inference.infer_long(args.ref_audio, args.ref_text, args.ref_lang, args.text, args.lang, chunk_length=args.chunk_length):

...
def load_audio_equivalent(filename: str, device: torch.device) -> tuple[torch.Tensor, int]:
    """
    非 torchaudio 的等效实现。

    Args:
        filename: 音频文件路径
        device: 目标计算设备 (cuda/cpu)

    Returns:
        audio: 形状为 (Channels, Time) 的 Tensor, float32
        sr: 采样率 (int)
    """
    ...
    # 1. 使用 librosa 加载
    # sr=None: 保持音频的原生采样率，不进行重采样（对齐 torchaudio 默认行为）
    # mono=False: 保持多声道结构，若为单声道则保持原样
    waveform_np, sr = librosa.load(filename, sr=None, mono=False)

    # 2. 形状对齐 (Shape Alignment)
    # torchaudio.load 始终返回 (Channels, Time)
    # librosa 对于单声道默认返回 (Time,)，需升维至 (1, Time)
    if waveform_np.ndim == 1:
        waveform_np = waveform_np[np.newaxis, :]

    # 3. 类型转换与设备迁移
    # librosa 加载默认为 float32，直接转换即可
    audio = torch.from_numpy(waveform_np)

    # 确保数据类型为 float32 (某些 librosa 版本或后端可能产生 float64)
